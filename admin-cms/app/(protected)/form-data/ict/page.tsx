...
const QUERY_KEY = ['ict-form-data'];

const ICT_COLUMNS = [
    { key: 'school', label: 'School' },
    { key: 'district', label: 'District' },
    { key: 'udise', label: 'UDISE' },
    { key: 'have_smart_tvs', label: 'Have Smart TVs?' },
    { key: 'have_ups', label: 'Have UPS?' },
    { key: 'have_pendrives', label: 'Have Pendrives?' },
    { key: 'ict_materials_working', label: 'ICT Materials Working?' },
    { key: 'smart_tvs_wall_mounted', label: 'Smart TVs Wall Mounted?' },
    { key: 'smart_class_in_routine', label: 'Smart Class in Routine?' },
    { key: 'school_routine', label: 'School Routine' },
    { key: 'weekly_smart_class', label: 'Weekly Smart Class' },
...
const ALL_COLUMNS = [{ key: 'sl', label: 'Sl No.' }, ...ICT_COLUMNS];
...
export default function ICTFormDataPage() {
    ...
    return (
        <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
            ...
            <motion.div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" variants={itemVariants}>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">ICT Form Data</h1>
                            <p className="text-slate-500 dark:text-slate-400 text-sm">View submitted form data for ICT</p>
                        </div>
                    </div>
            ...
            {/* Loading state — replaces the table */}
            {isLoading && (
                <motion.div
                ...
                >
                    <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <span className="text-slate-500 dark:text-slate-400 text-base">Loading ICT form data...</span>
                    </div>
                </motion.div>
            )}

            {/* Error state — replaces the table */}
            {!isLoading && isError && (
                <motion.div
                    className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/50 shadow-xl py-20"
                    variants={cardVariants}
            ...
            {/* Table Card — only when data is loaded */}
            {!isLoading && !isError && (
                <motion.div
