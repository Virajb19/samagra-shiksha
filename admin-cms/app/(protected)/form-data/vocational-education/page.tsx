...
import {
    vocationalFormDataFirestore,
    type VocationalFormDataRow,
} from '@/services/firebase/vocational-education-form-data.firestore';
...
const VOCATIONAL_COLUMNS = [
    { key: 'school', label: 'School' },
    { key: 'district', label: 'District' },
    { key: 'udise', label: 'UDISE' },
    { key: 'submitted_by_name', label: 'Submitted By' },
    { key: 'trade', label: 'Trade' },
    { key: 'class_9_boys', label: 'Class 9 Boys' },
    { key: 'class_9_girls', label: 'Class 9 Girls' },
    { key: 'class_10_boys', label: 'Class 10 Boys' },
    { key: 'class_10_girls', label: 'Class 10 Girls' },
    { key: 'class_11_boys', label: 'Class 11 Boys' },
    { key: 'class_11_girls', label: 'Class 11 Girls' },
    { key: 'class_12_boys', label: 'Class 12 Boys' },
    { key: 'class_12_girls', label: 'Class 12 Girls' },
    { key: 'is_lab_setup', label: 'Lab Setup?' },
    { key: 'lab_photo', label: 'Lab Photo' },
    { key: 'lab_not_setup_reason', label: 'Lab Not Setup Reason' },
    { key: 'is_guest_lecture_done', label: 'Guest Lecture?' },
    { key: 'guest_lecture_photo', label: 'Guest Lecture Photo' },
    { key: 'guest_lecture_not_done_reason', label: 'Guest Lecture Reason' },
    { key: 'is_industrial_visit_done', label: 'Industrial Visit?' },
    { key: 'industrial_visit_photo', label: 'Industrial Visit Photo' },
    { key: 'industrial_visit_not_done_reason', label: 'Industrial Visit Reason' },
    { key: 'is_internship_done', label: 'Internship?' },
    { key: 'internship_report', label: 'Internship Report' },
    { key: 'internship_not_done_reason', label: 'Internship Reason' },
    { key: 'best_practices', label: 'Best Practices' },
    { key: 'best_practice_photos', label: 'Best Practice Photos' },
    { key: 'success_stories', label: 'Success Stories' },
    { key: 'success_story_photos', label: 'Success Story Photos' },
];

const ALL_COLUMNS = [{ key: 'sl', label: 'Sl No.' }, ...VOCATIONAL_COLUMNS];
...
export default function VocationalFormDataPage() {
    ...
    useEffect(() => { vocationalFormDataFirestore.clearCursorCache(); setCurrentPage(1); }, [district, date]);

    const {
        data, isLoading, isFetching, isError, refetch, isPlaceholderData,
    } = useQuery({
        queryKey: ['vocational-form-data', currentPage, activeDistrict, activeDate],
        queryFn: () => vocationalFormDataFirestore.fetchPage(currentPage, activeDistrict, activeDate),
    ...
    useEffect(() => {
        if (currentPage < totalPages) {
            queryClient.prefetchQuery({
                queryKey: ['vocational-form-data', currentPage + 1, activeDistrict, activeDate],
                queryFn: () => vocationalFormDataFirestore.fetchPage(currentPage + 1, activeDistrict, activeDate),
            });
        }
            ...
            <motion.div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" variants={itemVariants}>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Vocational Education</h1>
                            <p className="text-slate-500 dark:text-slate-400 text-sm">View submitted form data for Vocational Education</p>
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    <RefreshTableButton queryKey={['vocational-form-data', currentPage, activeDistrict, activeDate]} isFetching={isFetching} />
                    <DownloadXlsxButton onDownload={async () => { const rows = await vocationalFormDataFirestore.fetchAll(activeDistrict, activeDate); downloadXlsx(rows, VOCATIONAL_COLUMNS, 'Vocational_Education_Form_Data'); }} disabled={!hasData} />
                </div>
            </motion.div>
            ...
            {isLoading && (
                ...
                >
                    <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <span className="text-slate-500 dark:text-slate-400 text-base">Loading vocational education form data...</span>
                    </div>
                </motion.div>
            )}
