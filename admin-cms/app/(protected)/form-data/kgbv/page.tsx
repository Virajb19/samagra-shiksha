...
const KGBV_COLUMNS = [
    ...
    { key: 'kgbv_type', label: 'KGBV Type' },
    { key: 'submitted_by_name', label: 'Submitted By' },
    { key: 'photo', label: 'Photo' },
    { key: 'activity', label: 'Activities' },
    { key: 'girl_participants', label: 'Girl Participants' },
    { key: 'girls_benefited', label: 'Girls Benefited' },
    { key: 'materials_used', label: 'Materials Used' },
    { key: 'instructor_name', label: "Instructor's Name" },
    { key: 'contact_number', label: 'Phone Number' },
    { key: 'best_practices', label: 'Best Practices' },
    { key: 'success_story', label: 'Success Story' },
];

const ALL_COLUMNS = [{ key: 'sl', label: 'Sl No.' }, ...KGBV_COLUMNS];

// Frozen / sticky column config
const FROZEN_KEYS = new Set(['sl', 'ebrc', 'district', 'kgbv_type']);
const FROZEN_STYLES: Record<string, { width: string; left: string }> = {
    sl: { width: 'w-[60px] min-w-[60px]', left: 'left-0' },
    ebrc: { width: 'w-[180px] min-w-[180px]', left: 'left-[60px]' },
    district: { width: 'w-[160px] min-w-[160px]', left: 'left-[240px]' },
    kgbv_type: { width: 'w-[140px] min-w-[140px]', left: 'left-[400px]' },
};
    ...
    return (
        <>
            <motion.button onClick={() => setOpen(true)}
                className="px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-full text-sm font-medium transition-all hover:scale-105"
                whileTap={{ scale: 0.95 }}>
                View
            </motion.button>
...
function renderCell(key: string, row: Record<string, unknown>) {
    ...
    if (key === 'photo' && typeof value === 'string' && value.startsWith('http')) {
        return <PhotoCell photos={[value]} />;
    }
...
export default function KGBVFormDataPage() {
            ...
            <motion.div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" variants={itemVariants}>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">KGBV Form Data</h1>
                            <p className="text-slate-500 dark:text-slate-400 text-sm">View submitted form data for KGBV</p>
                        </div>
                    </div>
            ...
            {isLoading && (
                ...
                >
                    <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <span className="text-slate-500 dark:text-slate-400 text-base">Loading KGBV form data...</span>
                    </div>
                </motion.div>
            )}
                            ...
                            <thead className="sticky top-0 z-50">
                                <tr>
                                    {ALL_COLUMNS.map((col, i) => {
                                        ...
                                        const isLastFrozen = col.key === 'kgbv_type';
                                        return (
