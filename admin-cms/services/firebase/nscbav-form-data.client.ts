/**
 * NSCBAV Form Data — Client-Side Firestore Service
 *
 * Cursor-based pagination using the client SDK.
 * Used by useQuery in the NSCBAV Form Data page.
 *
...
export interface NSCBAVFormDataRow {
    id: string;
    ebrc: string;
    district: string;
...
export interface NSCBAVFormDataPage {
    rows: NSCBAVFormDataRow[];
    totalCount: number;
    totalPages: number;
    page: number;
}
...
function toRow(docId: string, d: DocumentData): NSCBAVFormDataRow {
    return {
        id: docId,
        ebrc: d.ebrc ?? '',
        district: d.district ?? '',
...
export function clearNSCBAVFormDataCursorCache() {
    cursorCache.clear();
}
...
/**
 * Fetch a page of NSCBAV form data using Firestore client SDK with cursor-cached pagination.
 *
 * @param page     - 1-based page number (default 1)
 * @param district - Optional district filter
 * @param date     - Optional date filter (YYYY-MM-DD string)
 */
export async function fetchNSCBAVFormDataPage(
    page: number = 1,
    district?: string,
    date?: string,
): Promise<NSCBAVFormDataPage> {
    await devDelay('read', 'fetching NSCBAV form data');
    await waitForAuthReady();
    const db = getFirebaseFirestore();
    const colRef = collection(db, 'nscbav_form_data');
...
/**
 * Fetch ALL NSCBAV form data rows (no pagination) for XLSX export.
 */
export async function fetchAllNSCBAVFormData(
    district?: string,
    date?: string,
): Promise<NSCBAVFormDataRow[]> {
    await waitForAuthReady();
    const db = getFirebaseFirestore();
    const colRef = collection(db, 'nscbav_form_data');
