    ...
    return (
        <View className="flex-1 bg-[#f0f4f8]">
            <View className="p-4 pb-2">
                <View className="flex-row items-center bg-white rounded-xl px-4 py-3 gap-3 border border-gray-200">
            ...
            >
                {filteredCirculars && filteredCirculars.length > 0 ? (
                    filteredCirculars.map((circular) => (
                        ...
                        >
                            <View className="flex-row justify-between items-center mb-3">
                                <View className="flex-row items-center px-2 py-1 rounded-[6px] border gap-1" style={{ backgroundColor: '#e8f4fd', borderColor: '#bae6fd' }}>
                                    <Ionicons name="document-text" size={14} color={BLUE} />
                                    <Text className="text-[10px] font-semibold uppercase" style={{ color: BLUE }}>{circular.circular_no}</Text>
                                </View>
                            ...
                            <View className="flex-row justify-between items-center mt-2">
                                ...
                                {circular.school?.name && (
                                    <Text className="text-xs font-medium" style={{ color: BLUE }}>{circular.school.name}</Text>
                                )}
                            </View>
                            {circular.file_url && (
                                <TouchableOpacity
                                    className="flex-row items-center mt-3 pt-3 border-t border-t-gray-200 gap-1"
                                    onPress={() => Linking.openURL(circular.file_url!)}
                                >
                                    <Ionicons name="attach" size={16} color={BLUE} />
                                    <Text className="text-sm font-medium" style={{ color: BLUE }}>View Attachment</Text>
                                </TouchableOpacity>
                            )}
                        </View>
                    ))
                    ...
                    <View className="flex-1 justify-center items-center py-16">
                        ...
                        <Text className="text-sm text-gray-400 text-center mt-2">No circulars available at the moment.</Text>
                    </View>
                )}
            </ScrollView>
        </View>
