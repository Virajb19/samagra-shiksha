    ...
    return (
        <ScrollView className="flex-1 bg-[#eaf0fb]" contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
            ...
            <View className="bg-white rounded-2xl p-5 items-center mb-5" style={{ elevation: 2 }}>
                ...
                <Text className="text-xl font-bold text-[#1a1a2e]">{user?.name || 'Teacher'}</Text>
                <View className="flex-row items-center mt-1 gap-1">
                    <Ionicons name="mail-outline" size={14} color="#6b7280" />
                    <Text className="text-sm text-gray-500">{user?.email || ''}</Text>
                </View>
                <View className="bg-[#dbeafe] px-5 py-1.5 rounded-full mt-3">
                    <Text className="text-sm font-semibold text-[#3b82f6]">Teacher</Text>
                </View>
            </View>
            ...
            <TouchableOpacity
                ...
                style={{ elevation: 1 }}
                onPress={() => router.push('/(protected)/teacher/edit-personal-details' as any)}
            >
