/**
 * Junior Engineer Circulars Tab Screen
...
export default function CircularsTabScreen() {
    ...
    const { data: circulars, isLoading, refetch, isRefetching } = useQuery<Circular[]>({
        queryKey: ['circulars'],
            ...
            <View className="flex-1 bg-[#f0f4f8] justify-center items-center px-6">
                <View className="rounded-2xl py-6 px-4 items-center w-full" style={{ borderWidth: 1.5, borderStyle: 'dashed', borderColor: BLUE, backgroundColor: '#e8f4fd' }}>
                    ...
                    <Text className="text-gray-500 text-sm text-center mt-2">
                        {!hasCompletedProfile ? 'Complete your profile to access circulars.' : 'Please wait for admin to verify your account.'}
                    </Text>
                </View>
            </View>
        );
    ...
    const filteredCirculars = circulars?.filter(c =>
        ...
        c.circular_no?.toLowerCase().includes(searchQuery.toLowerCase())
    ) || [];
        ...
        >
            ...
            {isLoading ? (
                ...
                <View className="items-center mt-10">
                    ...
                    <Text className="text-gray-400 mt-3">No circulars found</Text>
                </View>
            ) : (
                filteredCirculars.map((circular) => (
                    <View key={circular.id} className="bg-white rounded-xl p-4 mb-3" style={{ elevation: 1 }}>
                        <View className="flex-row items-center mb-1">
                            <View className="bg-blue-100 px-2 py-0.5 rounded">
                                <Text className="text-xs font-semibold text-blue-700">{circular.circular_no}</Text>
                            </View>
                        </View>
                        <Text className="text-base font-semibold text-gray-900 mt-1">{circular.title}</Text>
                        {circular.description && <Text className="text-sm text-gray-500 mt-1" numberOfLines={2}>{circular.description}</Text>}
                        {circular.file_url && (
                            <TouchableOpacity
