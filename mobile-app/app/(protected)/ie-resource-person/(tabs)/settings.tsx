/**
 * IE Resource Person Settings Tab
 */
        ...
        try {
            await Share.share({
                ...
                message: Platform.OS === 'ios'
                    ? 'Check out Samagra Shiksha Nagaland app for managing school activities, events, and more!'
    ...
    return (
        <ScrollView className="flex-1 bg-[#eaf0fb]" contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
            <View className="bg-white rounded-2xl p-5 items-center mb-5" style={{ elevation: 2 }}>
                ...
                <Text className="text-xl font-bold text-[#1a1a2e]">{user?.name || 'IE Resource Person'}</Text>
                <View className="flex-row items-center mt-1 gap-1">
                    <Ionicons name="mail-outline" size={14} color="#6b7280" />
                    <Text className="text-sm text-gray-500">{user?.email || ''}</Text>
                </View>
                <View className="bg-[#dbeafe] px-5 py-1.5 rounded-full mt-3">
                    <Text className="text-sm font-semibold text-[#3b82f6]">IE Resource Person</Text>
                </View>
            </View>

            <TouchableOpacity className="bg-white rounded-2xl p-4 flex-row items-center mb-3" style={{ elevation: 1 }} onPress={() => router.push('/(protected)/ie-resource-person/edit-personal-details' as any)}>
            ...
            <TouchableOpacity className="bg-white rounded-2xl p-4 flex-row items-center mb-3" style={{ elevation: 1 }} onPress={() => router.push('/(protected)/ie-resource-person/helpdesk' as any)}>
            ...
            >
                <View className="flex-1 justify-center items-center bg-black/50 px-8">
                    <View className="bg-white rounded-2xl p-6 w-full max-w-[320px]" style={{ shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 12, elevation: 8 }}>
