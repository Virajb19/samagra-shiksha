...
export const RegisterSchema = z
  .object({
    ...
    confirmPassword: trimmedString.pipe(
...
export type IEResourcePersonProfileFormData = z.infer<
  typeof IEResourcePersonProfileSchema
>;
...
export type JuniorEngineerProfileFormData = z.infer<
  typeof JuniorEngineerProfileSchema
>;
...
export const ICTFormPage1Schema = z.object({
  haveSmartTvs: z.enum(["Yes", "No"], {
    message: "Please select whether Smart TVs are provided",
  }),
  haveUps: z.enum(["Yes", "No"], {
    message: "Please select whether UPS are provided",
  }),
  havePendrives: z.enum(["Yes", "No"], {
    message: "Please select whether e-Content Pendrives are provided",
  }),
  ictMaterialsWorking: z.enum(["Yes", "No"], {
    message: "Please select whether ICT materials are in working condition",
  }),
  smartTvsWallMounted: z.enum(["Yes", "No"], {
    message: "Please select whether Smart TVs are wall mounted",
  }),
  smartTvsLocation: z.enum(["Classrooms", "Other Rooms"], {
    message: "Please select where Smart TVs are installed",
  }),
  ictMaterialPhotos: z
    .array(z.string())
    .min(1, "Please upload at least 1 photo of ICT materials")
...
export const ICTFormPage2Schema = z.object({
  smartClassInRoutine: z.enum(["Yes", "No"], {
    message: "Please select whether Smart Class is in school routine",
  }),
  schoolRoutinePdf: z
    .string({ message: "Please upload school routine PDF" })
    .min(1, "Please upload school routine PDF (max 5 MB)"),
  ...
  hasLogbook: z.enum(["Yes", "No"], {
    message: "Please select whether logbook is maintained",
  }),
  logbookPdf: z
    .string({ message: "Please upload logbook PDF" })
    .min(1, "Please upload logbook PDF (max 5 MB)"),
});
...
export const ICTFormPage3Schema = z.object({
  ...
  smartTvsOtherPurposes: z.enum(["Yes", "No"], {
    message: "Please select whether Smart TVs are used for other purposes",
  }),
  isSmartClassBenefiting: z.enum(["Yes", "No"], {
    message: "Please select whether Smart Class is benefiting students",
  }),
  benefitComment: z.string().optional(),
  teacherImpact: z
    .string()
    .min(1, "Please describe the impact on students' performance"),
  howProgramHelped: z
    .string()
    .min(1, "Please describe how the program helped teachers"),
 ...
 */
export const ICTFormSchema = ICTFormPage1Schema.extend(
  ICTFormPage2Schema.shape,
).extend(ICTFormPage3Schema.shape);
...
export const LibraryFormSchema = z.object({
  ...
  isLibraryAvailable: z.enum(["Yes", "No"], {
    message: "Please select whether library is available",
  }),
  isChildFriendly: z.enum(["Yes", "No"], {
    message: "Please select whether library is child friendly",
  }),
  hasProperFurniture: z.enum(["Yes", "No"], {
    message: "Please select whether library has proper furniture",
  }),
  hasManagementCommittee: z.enum(["Yes", "No"], {
    message: "Please select whether management committee is constituted",
  }),

  // Teacher in-charge
  libraryTeacherName: z
    .string()
    .min(1, "Please enter Library Teacher In-charge name"),

  // Reading corner
  hasReadingCorner: z.enum(["Yes", "No"], {
    message: "Please select whether school has reading corner",
  }),
  ...
  hasReadersClub: z.enum(["Yes", "No"], {
    message: "Please select whether school has readers club",
  }),
  hasWeeklyLibraryPeriod: z.enum(["Yes", "No"], {
    message: "Please select whether classes have library period",
  }),
  ...
  receivedBooksFromSamagra: z.enum(["Yes", "No"], {
    message: "Please select whether books were received",
  }),
  ...
  innovativeInitiative: z
    .string()
    .min(1, "Please describe the innovative initiative"),
  ...
  studentPhotos: z
    ...
    .min(
      1,
      "Please upload at least 1 photo of students utilising library books",
    )
    .max(10, "Maximum 10 photos allowed"),
  logbookPhotos: z
    .array(z.string())
    .min(1, "Please upload at least 1 photo of Library Logbook")
    .max(10, "Maximum 10 photos allowed"),
});
...
export const ScienceLabFormSchema = z.object({
  ...
  studentPhotos: z
    .array(z.string())
    .min(1, "Please upload at least 1 photo of students using kits")
    .max(10, "Maximum 10 photos allowed"),
  logbookPhotos: z
    .array(z.string())
    .min(1, "Please upload at least 1 photo of Logbook")
    .max(10, "Maximum 10 photos allowed"),
});
...
export const IESchoolVisitFormSchema = z.object({
  ...
  activitiesTopics: z
    .string()
    .min(1, "Please enter activities / topics covered"),
  ...
  wasGoalAchieved: z.enum(["Yes", "No"], {
    message: "Please select whether the goal was achieved",
  }),
  geoTaggedPhotos: z
    .array(z.string())
    .min(1, "Please upload at least 1 geo-tagged photo")
    .max(10, "Maximum 10 photos allowed"),
});
...
export const ProjectStatusUpdateSchema = z.object({
  completionStatus: z
    .string()
    .min(1, "Please select project completion status"),
  comment: z.string().optional(),
  photos: z
    .array(z.string())
    .min(1, "Please upload at least 1 photo of project status")
    .max(10, "Maximum 10 photos allowed"),
});

export type ProjectStatusUpdateFormData = z.infer<
 ...
 */
export const VocationalEducationFormSchema = z
  .object({
    ...
    // Lab setup
    isLabSetup: z.enum(["Yes", "No"], {
      message: "Please select whether lab is set up",
    }),
    labPhoto: z.string().optional(),
    labNotSetupReason: z.string().optional(),

    // Guest lecture
    isGuestLectureDone: z.enum(["Yes", "No"], {
      message: "Please select whether guest lecture is done",
    }),
    guestLecturePhoto: z.string().optional(),
    guestLectureNotDoneReason: z.string().optional(),

    // Industrial visit
    isIndustrialVisitDone: z.enum(["Yes", "No"], {
      message: "Please select whether industrial visit is done",
    }),
    industrialVisitPhoto: z.string().optional(),
    industrialVisitNotDoneReason: z.string().optional(),

    // Internship
    isInternshipDone: z.enum(["Yes", "No"], {
      message: "Please select whether internship is done",
    }),
    ...
    // Best practices
    bestPractices: z.string().min(1, "Please enter best practices"),
    bestPracticePhotos: z
      .array(z.string())
      .min(1, "Please upload at least 1 photo of best practices")
      .max(10, "Maximum 10 photos allowed"),

    // Success stories
    successStories: z.string().min(1, "Please enter success stories"),
    successStoryPhotos: z
      .array(z.string())
      .min(1, "Please upload at least 1 photo of success stories")
      .max(10, "Maximum 10 photos allowed"),
  })
  .superRefine((data, ctx) => {
    ...
    if (
      data.isGuestLectureDone === "No" &&
      (!data.guestLectureNotDoneReason ||
    ...
    if (
      data.isIndustrialVisitDone === "No" &&
      (!data.industrialVisitNotDoneReason ||
...
export type VocationalEducationFormData = z.infer<
  typeof VocationalEducationFormSchema
>;
...
export const EditPersonalDetailsSchema = z.object({
  ...
  phone: trimmedString.pipe(
    z
      ...
      .refine(
        (v) => /^\d{10}$/.test(v) || (/^91\d{10}$/.test(v) && v.length === 12),
        "Enter a valid 10-digit phone number",
      )
      .transform((v) =>
        v.length === 12 && v.startsWith("91") ? v.slice(2) : v,
      ),
  ),
  gender: GenderEnum,
});

export type EditPersonalDetailsFormData = z.infer<
