...
const FORM_TYPE_TO_ACTIVITY_NAME: Record<string, string> = {
  "ICT": "ICT",
  "Library": "Library",
...
interface ActiveFormWindow {
  formName: string;
  startingDate: string;
  endingDate: string;
}

async function getActiveFormWindows(): Promise<ActiveFormWindow[]> {
  const snap = await db.collection("activity_forms").where("status", "==", "Open").get();
  const windows: ActiveFormWindow[] = [];
  ...
  for (const doc of snap.docs) {
    ...
    if (!startingDate || !endingDate) continue;
...
async function getDistrictNames(): Promise<Map<string, string>> {
  const snap = await db.collection("districts").get();
  const map = new Map<string, string>();
  snap.forEach((doc) => map.set(doc.id, (doc.data().name as string) || doc.id));
  return map;
}

async function getSchoolNames(): Promise<Map<string, { name: string; district_id: string }>> {
  const snap = await db.collection("schools").get();
  const map = new Map<string, { name: string; district_id: string }>();
  snap.forEach((doc) => {
    const d = doc.data();
    map.set(doc.id, { name: (d.name as string) || doc.id, district_id: (d.district_id as string) || "" });
  });
  return map;
}
...
async function computeNonSubmitters(
  mapping: FormMapping,
  window: ActiveFormWindow,
  districtNames: Map<string, string>,
  schoolNames: Map<string, { name: string; district_id: string }>,
  ...
  const [submissionSnap, usersSnap] = await Promise.all([
    db.collection(mapping.collection)
      ...
      .get(),
    db.collection("users")
      .where("role", mapping.roles.length === 1 ? "==" : "in",
        mapping.roles.length === 1 ? mapping.roles[0] : mapping.roles)
  ...
  submissionSnap.forEach((doc) => {
    const d = doc.data();
    if (d.submitted_by) submittedUserIds.add(d.submitted_by as string);
    if (d.user_id) submittedUserIds.add(d.user_id as string);
  });
  ...
  usersSnap.forEach((doc) => {
    ...
    if (submittedUserIds.has(userId)) return;

    // Responsibility filter for teachers
    if (mapping.responsibility) {
      const responsibilities = (d.responsibilities as string[]) ?? [];
      if (!responsibilities.includes(mapping.responsibility)) return;
    }

    const schoolId = (d.school_id as string) || "";
    const districtId = (d.district_id as string) || "";
    ...
    records.push({
      ...
      user_id: userId,
      user_name: (d.name as string) || "Unknown",
...
async function batchWrite(records: MissingRecord[]): Promise<number> {
...
async function computeMissingFormsCore(): Promise<{ deleted: number; written: number }> {
  ...
  const tasks: Promise<MissingRecord[]>[] = [];
 ...
 */
export const computeMissingFormsScheduled = onSchedule(
 ...
 */
export const computeMissingFormsHttp = onRequest(
